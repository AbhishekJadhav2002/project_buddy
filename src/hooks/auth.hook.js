...
import { loginUser, signUpUser, getProfile, getUserProfile } from '../api';
...
function useGetUserProfile(username) {
  const navigate = useNavigate()
  const { isLoading, isError, data, error } = useQuery({ queryKey: ['userprofile'], queryFn: getUserProfile(username), enabled:!!username })
  if (isError) {
    const parsedError = errorParser(error)
    if (parsedError.server) toast.error(parsedError.server, { autoClose: 5000 })
    else {
