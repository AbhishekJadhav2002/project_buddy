...
import { useGetProfile, useGetUserProfile } from '../hooks/auth.hook';
...
function Navbar() {
  ...
  const {data:userdata,isLoading: isprofileLoading}=useGetUserProfile(data?.data.username);
                ...
                <div className={`absolute right-0 top-full mt-3 w-64 p-3 bg-white rounded-lg shadow-xl border border-gray-300 ${!profileOpen && 'hidden'}`}>
                  <div className='flex items-center'>
                    ...
                    <div className='flex flex-col justify-center flex-1 pl-3'>
                      <code className='text-gray-900 font-medium text-lg'>{`${data?.data.first_name} ${data?.data.last_name}`}</code>
                      <p className='text-gray-500 text-sm'>@{isprofileLoading?'Loading':userdata?.data.title}</p>
                    </div>
                  </div>
                  <button className='mx-1/4 text-gray-900 font-medium text-sm py-2 px-3 bg-gray-100 hover:bg-blue-400 rounded-lg mt-3' onClick={() => profileNavigator(`/profile/${data.data.username}`)}>View Profile</button>
                  <br />
